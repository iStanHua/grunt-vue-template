; (function () {
    var stan = {
        ...
        * @param {number} value 滚动位置值
        ...
        /**
         * 滚动加载
         * @param {object} active 是否第一次执行成功回调
         * @param {function} success 成功回调
         */
        scrollLoad: function (options) {
            options.active = options.active == undefined ? true : options.active;
            if (options.active && typeof options.success === 'function') {
                options.success();
            }
            window.onscroll = function () {
                var _scrollTop = app.scrol.get();
                ...
                if (_scrollTop >= (parseInt(_scrollHeight) - parseInt(_clientHeight))) {
